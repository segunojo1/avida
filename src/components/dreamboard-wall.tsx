...
export const createMasonryLayout = (items: any[], columns: number) => {
  ...
  items.forEach((item) => {
    const height =
      item.card === "yellow" ? 215 : item.card === "blue" ? 400 : 500;
...
const DreamboardWall = ({ cards }: { cards: DreamCard[] }) => {
  ...
  const filteredCards = useMemo(() => {
    return cards?.filter((card) => {
      if (categoryFilter && card.vibe !== categoryFilter) {
        return false;
      }
      ...
      if (searchQuery.trim()) {
        ...
        return (
          ...
          card.vibe?.toLowerCase().includes(query)
        );
      }
              ...
              <div key={`column-${colIndex}`} className="flex flex-col gap-6">
                {column.map((card) => (
                  <motion.div
                    ...
                    className={`${
                      CARD_SIZES[card.card as keyof typeof CARD_SIZES]
                    } 
                `}
                  >
                    <DreamboardCard {...card} />
